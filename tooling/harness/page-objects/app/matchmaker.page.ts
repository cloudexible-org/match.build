...
export class ConversationPage {
  ...
  /**
   * The composer's own Send, scoped to it. A drafted reply's card carries a
   * Send too (`suggestion-accept`), so an unscoped `getByRole` matches two
   * buttons whenever a draft is on offer and fails strict mode — which is a
   * state every conversation reaches once the assistant is running.
   */
  getSendButton() {
    return this.getComposer().getByRole("button", { name: /^Send/ });
  }
