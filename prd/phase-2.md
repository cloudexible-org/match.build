...
| Per-matchmaker AI budget | — | **Not built, and now the gap that matters most** | Both the limit *and* what happens when it's hit: degrade to no suggestions, or tell them? Not a number waiting for a pilot. A message now costs up to two generations rather than one — a draft and a reconciliation — plus a third every twentieth message the matchmaker sends. This is the only thing between a busy book and a bill nobody chose, and it is the last piece of §6 with nothing behind it. |
