...
* **Page Objects Pattern:** Whenever a new page is created in `apps/app` or `apps/admin`, or a significant UI component is added to `packages/ui`, you must:
    1.  Update or create the corresponding file in `tooling/harness/page-objects/<app>/`, and list it in that app's `index.ts`.
    2.  Ensure selectors are resilient (prefer data-attributes like `data-testid` over CSS classes).
  Page objects live in the harness, not the suite, because the marketing captures drive the app through the same ones — when a `data-testid` moves, a demo clip follows the suite instead of silently filming the wrong element.
* **Everything that drives a browser lives in `tooling/`,** in three packages: `tooling/harness` (`@repo/harness`) owns the world — the base Playwright config, the local Convex backend, per-run ports, the seed, sign-in and every page object — and registers no tests of its own; `tooling/e2e` is the suite, nothing but `specs/` and a config that spreads the base; `tooling/marketing` films demo clips of the app for the pitch deck, on demand and never in CI. A consumer imports the world through `@repo/harness/*` and never by relative path. The split is what stops `pnpm test:e2e` from ever running a capture, and a capture's assert-nothing style from being mistaken for a test.
* **E2E / Integration Tests:** `tooling/e2e` holds one Playwright project per app: `app` drives the Vite app (`apps/app`), `admin` drives the platform admin app (`apps/admin`), `www` drives the Next.js marketing site (`apps/www`), each on a port allocated for the run, and `app-convex` / `admin-convex` drive `apps/app` and `apps/admin` against a real, seeded Convex local backend. Specs live in `tooling/e2e/specs/<app>/` and page objects in `tooling/harness/page-objects/<app>/`; each project pins its own `baseURL`, so put a spec in the directory of the app it asserts against. Ensure that new features are accompanied by a Playwright test script utilizing the updated page objects.
* **Marketing captures are not tests.** A capture in `tooling/marketing/captures/` asserts nothing — it photographs the app as timed frames plus a manifest, which `render:demo` turns into an mp4/GIF. Keep assertions out of them and coverage in `tooling/e2e/specs/`. See `docs/demo-videos.md`.
* **Test data:** a spec file seeds its own world in `beforeAll` with `seedScenario()` (`tooling/harness/scenario.ts`), namespaced so no other file can see it, and signs in with `signInAs()` (`tooling/harness/session.ts`) rather than through the UI. The baseline seed (`seed/e2e/fixture.ts`) is shared and read-only. Tests in a file run serially and may share its data; a test that writes owns its own rows. Keep a file under ~10 tests — see `docs/e2e-architecture.md` §8.
* **Convex-backed specs:** Anything asserting on backend data belongs in `tooling/e2e/specs/app-convex/`, seeded from `packages/api/convex/seed/e2e/fixture.ts`. The suite talks only to a **local, anonymous** Convex deployment, whose port is allocated fresh for every run — never hard-code 3210 or trust the port recorded in `.convex/local/default/config.json`, and read `docs/e2e-architecture.md` §1a and §1c before touching that harness: seeding wipes the database, and a wrong port can wipe a *different project's*.
...
* **A directory that registers no functions is allowed, and has to earn it.** `convex/profiles/` is the one: it holds the entry type, validation and write engine that `convex/candidateProfiles/` and `convex/matchmakerProfiles/` both build on, because "an agent never overwrites what a person typed" has to mean the same thing in both. It defines no `query`, `mutation` or `action`. Reach for this only when two domains would otherwise hold two copies of one rule; a third directory for convenience is a worse shape than the duplication.
* **A directory that registers no functions is allowed, and has to earn it.** `convex/profiles/` is the one: it holds the entry type, validation and write engine that `convex/candidateProfiles/` and `convex/matchmakerProfiles/` both build on, because "an agent never overwrites what a person typed" has to mean the same thing in both. It defines no `query`, `mutation` or `action`. Reach for this only when two domains would otherwise hold two copies of one rule; a third directory for convenience is a worse shape than the duplication.
* **Access helpers:** every function derives who the caller is and which tenant they may touch from `requireUser` (`users/helpers.ts`), `requireMatchmaker` (`matchmakers/helpers.ts`) or `requireCandidateSelf` (`candidates/helpers.ts`), never from arguments alone. The one exception is `convex/admin/`, which serves the platform admin app across every tenant: those functions start with `requirePlatformAdmin` (`admin/helpers.ts`), which checks the caller's verified email against the `PLATFORM_ADMIN_EMAILS` deployment env var. Any change to audited state calls `recordAudit` (`audit/helpers.ts`) in the same mutation. See `prd/phase-1.md` §5 and §9.
